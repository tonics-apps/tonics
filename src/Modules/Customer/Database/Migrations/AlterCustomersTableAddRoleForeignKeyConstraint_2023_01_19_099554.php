...
class AlterCustomersTableAddRoleForeignKeyConstraint_2023_01_19_099554 extends Migration {
    ...
    {
        try {
            $this->getDB()->run("ALTER TABLE `{$this->tableCustomer()}` DROP COLUMN `role`;");
            $this->getDB()->run("ALTER TABLE `{$this->tableCustomer()}` ADD COLUMN `role` INT AFTER `user_password`");
            $this->getDB()->run("ALTER TABLE `{$this->tableCustomer()}` ADD FOREIGN KEY (`role`) REFERENCES `{$this->tableRole()}`(`id`) ON UPDATE CASCADE ON DELETE RESTRICT;");
