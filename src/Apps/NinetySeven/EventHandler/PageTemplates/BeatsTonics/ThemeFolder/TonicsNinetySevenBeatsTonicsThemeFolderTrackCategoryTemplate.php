...
class TonicsNinetySevenBeatsTonicsThemeFolderTrackCategoryTemplate implements PageTemplateInterface, HandlerInterface
{
    ...
    public function handleTemplate(OnPageTemplate $pageTemplate): void
    {
        ...
        1 as is_track, CONCAT_WS('/', '/tracks', t.slug_id, t.track_slug) as _link")
                    ->From("{$this->getTrackTable()} t")
                    ->Join("{$this->getTrackGenreTable()} tg", "tg.fk_track_id", "t.track_id")
                    ->Join("{$this->getGenreTable()} g", "g.genre_id", "tg.fk_genre_id")
                    ->Join("{$this->getTrackTracksCategoriesTable()} ttc", "t.track_id", "ttc.fk_track_id")
                    ->Join("{$this->getTrackCategoriesTable()} ct", "ttc.fk_track_cat_id", "ct.track_cat_id")
                    ->Join("{$this->getLicenseTable()} tl", "tl.license_id", "t.fk_license_id")
                    ->Join("{$this->getArtistTable()} ta", "ta.artist_id", "t.fk_artist_id")
                    ->WhereEquals('ct.track_cat_id', $mainTrackData->track_cat_id)
                    ->Raw('UNION')
                    ->Select("ct.track_cat_id as id, ct.slug_id, ct.track_cat_name as _name,
        (SELECT COUNT(*) FROM {$this->getTrackTracksCategoriesTable()} ttc
        INNER JOIN {$this->getTrackTable()} t ON ttc.fk_track_id = t.track_id
