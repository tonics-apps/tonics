...
use App\Apps\TonicsCloud\Library\LXD\Repositories\Certificate;
use App\Apps\TonicsCloud\Library\LXD\Repositories\Instance;
use App\Apps\TonicsCloud\Library\LXD\Repositories\Operation;
...
class Client
{
     ...
     *
     * ```
     * // // only use the `LXDHelper::decryptCertificate()` method if you used the helper to encrypt it in the first place
     * certAndKey = [
    ...
    {
        if (isset($this->getResponse()->status_code)){
            return $this->getResponse()->status_code === self::ERROR_FAILURE || $this->getResponse()->status_code === self::ERROR_CANCELED;
        }
     ...
     * @throws \Exception
     */
    public function sendRequest(string $url, string $method, array|string $opts = [], array $headers = []): \stdClass|null
    {
        if (is_string($opts)){
            $post_fields = $opts;
        } else {
     ...
     * @return Client
     */
    public function setResponse(?\stdClass $response): Client
    {
